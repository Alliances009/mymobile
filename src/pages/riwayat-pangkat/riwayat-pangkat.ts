...
/**
 * Generated class for the RiwayatPangkatPage page.
 *
...
@Component({
  selector: 'page-riwayat-pangkat',
  templateUrl: 'riwayat-pangkat.html',
})
export class RiwayatPangkatPage {
	data_list:any;
  data:any;
  page:any;
  ...
  ionViewDidLoad() {
   ...
   getData(pid) {
      ...
      this.app_config.requestAjaxGet(this.app_config.data.site_url+"/pegawai/drh/pangkat/App/get",params,function(result){

      	 let data = [];
      	 result.data.forEach(function(row){      	 	
           if (row.TMTPANGKAT != null) {             
             let tgl_tmt = me.app_config.split_date_indo(row.TMTPANGKAT,'-',1);
             row.TMTPANGKAT_TANGGAL = tgl_tmt[0];
             row.TMTPANGKAT_BULAN = tgl_tmt[1].substr(0,3);
             row.TMTPANGKAT_TAHUN = tgl_tmt[2];
           }
